// ===============================================================
// CUDA arrays support normalized coordinates, and everything else
// ===============================================================
...
void betweenKernel(float* deviceResult, cudaTextureObject_t texObj, int size)
{
  ...
  // In this case, not using lerp, the position in this region is not important for the interpolation
  float textIdx = idx;

  // Read from texture and write to global memory
  if (idx < size)
  {
    deviceResult[idx] = 2 * tex1D<float>(texObj, textIdx);
  }
}
